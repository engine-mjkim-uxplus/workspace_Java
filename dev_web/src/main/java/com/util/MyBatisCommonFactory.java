 ...
 * 바로 갈고리한다음 토드나 오렌지 같은 툴을 이용해서 즉시 실행해 볼 수도 있는 것이다.
 ...
 * 
 * 자바설정 - Properties
 * XML설정 - spring2.0 ~ spring3.0주류(어노테이션은 2.5부터 제공은 됨 - 많이 부족함 - servlet에 의존적이다)
 * 어노테이션 설정 - 자바설정 - 그레이들방식(코틀린사용-2019구글공식 인정- jetbrain)
 * 
 * 
 */
public class MyBatisCommonFactory {
	...
	// 메소드를 통한 객체 주입 코드(디자인패턴적용, 싱글톤, 의존성주입, 제어역행, 개발 방법론(MVC, MVP, MVVM)
	// 인스턴스화 5가지 유형
	public static void init() {
		try {
			...
			if(sqlSessionFactory == null) { // null이 아닐때만 객체 주입을 새로 받는다(싱글톤)
