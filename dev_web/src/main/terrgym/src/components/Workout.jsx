...
function Workout(props) {
  ...
  // 태그안에 {}써서 자바스크립트 코드 작성
  // {}안에 이벤트 함수나 자료구조를 넘길 수 있다
  // 이벤트 소스는 Workout.jsx에서 감지됨 그러나 처리는 상위 요소로 전달하여 처리함
  // 왜지? ( 캡처링- 상위에서 하위로, 버블링- 하위에서 상위로
  // 그리고 그안에 count가 있다. - count 수정해야함
  // 이러한 이류로 리액트 코딩에서는 상위노드에서 이벤트 처리함
  const handleIncrement = (e) => {
    // 이벤트 발생 소스는 여기(Worktout.jsx(에 있지만 이벤트 처리는 workoutApp.jsp에서 함
    // 함수의 파라미터로 값을 넘길때 propse 뒤에 오는 이름은 상위노드에서 속성이름을 작성함
