...
// props는 컴포넌트와 컴포넌트 사이에 정보를 주고 받을 때 사용, 생략가능
function WorkoutApp() {
  // Array타입 -> XXX[0].id, XXX[1].id, XXX[2].id
  const [items, setItems] = useState([
    { id: 1, name: "벤치프레스", count: 0 },
    { id: 2, name: "랫풀다운", count: 0 },
    { id: 3, name: "스쿼트", count: 0 },
    // 이 부분이 자바 즉 스프링과 만나는 부분. json이어야 한다
  ]);
  const handleIncrement = (workout) => {
    console.log(`workout: ${workout.name}`);
    // 사용자가 선택한 로우의 인데스 값을 알아낸다
    const index = items.indexOf(workout); //  0, 1, 2, -1은 없다라는 의미
    ...
    // useState훅을 사용하면 효율적인 처리가 가능하다. 상태가 변한 경우에만 새로 처리를 함 / 변한 부분에 대해서만 가상의 돔을 만들어서 새로 처리를 한다
    // 메모리 사용도 줄어든다
    setItems([...items]);
  };
  ...
  return (
    <Workouts //
      workouts={items} // 태그의 속성으로 값을 넘길 수 있다 - props 활용
...
// rsf : function으로 자동완성
// 결론적으로 부모태그에서 하위태그로는 항상 언제든지 prpops를 통해서 값을 전달할 수 있다
// 다만 매번 파일마다 props를 전달 해야 하므로 피로도가 높다 - 그래서 리덕스가 출현
// 이벤트 처리도 위임하거나 전달이 가능하다
// 자바스크립트에서는 함수도 객체이기 때문에 가능하다
