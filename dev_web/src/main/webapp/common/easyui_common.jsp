...
// jsp에서 작성된 스크립트릿은 servlet전환 시 service() 메서드 안에 들어간다
// 스크립틀릿 안에 메소드를 선언할 수 없다 (자바스크립트 안에서는 가능하다)
// JSP = html+css+js(브라우저-인터프리터-로컬-다운로드)+java(백엔드-서버주체-서버측 처리된 결과가 다운로드됨)
// 왜 중요한 걸까? - 시간차
// 서블릿의 라이프사이클 - 스레드관리, 관리되는 주체 객체(서블릿)
// 위 3가지 중에서 개발자가 신경써야 하는 것은?? - service(req,res) -> doGet, doPost(구분할 수 있다) - destroy()
// 톰캣 서버가 init()과 destroy() 책임진다 - 개발자 책임이 아니다
// a.jsp -> a_jsp.java -> a_jsp.class -> 브라우저 쓰기
//       jsp컨테이너                servlet-api.jar(서블릿엔진)
// 왜 서블릿이어야 하는가?
