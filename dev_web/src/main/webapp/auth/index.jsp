...
<% 
	MemberVO mVO = (MemberVO)session.getAttribute("mVO"); // 세션에 담기는 값은 반환형이 Object
 	String s_id = null;
	String s_name = null;
	if(mVO != null){
	s_id = mVO.getMem_id();
		...
		function memberList(){
			...
			$("#dg_member").datagrid({
			   ...
			   ,onDblClickCell: function(index,field,value){
					console.log(index+", "+field+", "+ value); // 3, BUTTON, 쪽지쓰기
					if("BUTTON"== field){
						alert("쪽지쓰기");
					}
				}
...
<!--################ 메뉴 영역 시작 ################-->
    <div style="margin:20px 0;"></div>
<%
	if(s_id !=null) { /* 사용자가 로그인 하였을 때만 메뉴를 보여준다 */
%>
...
<!--################ 메뉴 영역  끝  ################-->
        ...
        <!--[[ 회원목록 출력 ]]-->	        	
        	    <table id="dg_member" class="easyui-datagrid"  style="width:700px;height:250px"
			        ...
			        <thead>
			            <tr>
			                <th data-options="field:'MEM_ID',width:80">Item ID</th>
			                <th data-options="field:'MEM_NAME',width:100">Product</th>
			                <th data-options="field:'MEM_ADDRESS',width:80,align:'left'">List Price</th>
			                <th data-options="field:'BUTTON',width:80,align:'center'">Unit Cost</th>
			            </tr>
			        </thead>
			    </table>
