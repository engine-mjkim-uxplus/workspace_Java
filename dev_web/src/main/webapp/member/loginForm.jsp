...
주소 - URL
장바구니, 찜 
쿠키 - 문자열로 저장된다 - 텍스트 - base2, base8, base16, base32, base64 -메모리주소번지와 관계가 있다, 개인pc에 저장(노출위험)
세션 - Object타입으로 다 담아진다 - 서버컴 캐시메모리에서 관리 - 노출의 위험 없다
둘다 저장소이다
session.seAttribute("s_name", "김유신");
...
<!-- 세션값이 null일때  -->
<%
// 로그인 하지 않고 페이지를 열었을 때 - 로그인 버튼 출력
	if(s_name == null){
%>
	<form id="f_login">
...
<%
	...
	else{ //로그인 성공했을 때 - 로그아웃 출력
%>
<!--
세션값이 존재할 때 로그아웃 버튼을 누르면 해당 사용자에 관한 모든 세션을 회수함  
  -->
	<%=s_name %>님 환영합니다
