...
// 입력할 때 빈칸을 제공하고 새로입력한 값을 저장해야 한다. - 업무 역할
// 수정일때는 기존의 등록된 정보에 대해서 일부를 변경지원한다.
// 입력한 내용 또는 수수정할 때 기존 정보를 제공하기 위해서라도 한건에 대한 상세조회가 선행되어야 한다.
public class Main_3 extends JFrame implements ActionListener{
   ...
   public void refreshData() {  // f5 새로고침과 같은 효과
      System.out.println("refreshData 호출 성공");
      // 새로고침 후에 자손 화면을 닫기(자손클래스의 자원 반납을 받는 메소드를 호출)
      dialog.dispose();
   }
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if(btnInsert == obj) {
    	  // 자손 클래스에서 부모클래스의 refreshData메소드를 호출하고싶다
