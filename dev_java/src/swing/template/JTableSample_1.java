...
public class JTableSample_1 implements ActionListener{
	// 윈도우 운영체제에서 창을 추가 생성하기
	JFrame jf = new JFrame();
	// 테이블의 헤더를 구성할 때 사용할 1차 배열 선언
	String cols[] = {"HTML","자바","총점"};
	// JTable은 테이블 양식만 제공할 뿐 실제 데이터를 담는 클래스는 DefalutTableModel이다.
	// 실제로 데이터를 가지고 있는 것은 2차 배열이다
	String data[][] = new String[1][3]; 
	// 만일 값을 접근하려면 DefaulTableModel안에 데이터 셋이 있다
	DefaultTableModel dtm = new DefaultTableModel(data, cols); // 배열 담기(배열, 열의 값), 테이블을 배열로 처리 할 수 있게 해준다
	...
	@Override
	public void actionPerformed(ActionEvent e) { // 자바에서 대신 주입해 준다. ActionEvent를 . 그래서 null이 아니다
