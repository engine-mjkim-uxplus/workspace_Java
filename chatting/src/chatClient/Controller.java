...
public class Controller {
	...
	public MemberVO action(MemberVO pmVO) {
		...
		// 아이디 중복체크
		 ...
		 else if (_UPDATE.equals(command)) {
			 logindao.editMember(pmVO);
		 }
		// 회원 검사
		 else if (_CHECK.equals(command)) {
			rsVO = logindao.memberCheck(pmVO);
		 }
		 else if (_DELETE.equals(command)) {
			 rsVO = logindao.withdrawal(pmVO);
		 }
		return rsVO;
	}
}
